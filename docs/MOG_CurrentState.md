...
**New-day auto-reset shipped (2026-05-26) — store-facing code reached all 8 stores.** The daily on-hand reset now fires automatically on a new day so the recap email + Order-History logging happen no matter what (Sebastian's goal: track usage without depending on someone tapping Reset). Three coupled changes plus a latent-bug fix: (1) the recap email is now a deduped side-effect of `commitLogAndReset`, so EVERY reset path emails exactly once per cycle (previously the PWA stale-gate and `api_commitReset_` never emailed); (2) the PWA auto-fires the reset on first open of a new day with a visible "Detected new day" overlay; (3) a new installable `onOpen` trigger (`dailyResetOnOpen_`) does the same when the Google Sheet is opened; (4) `buildHomeDashboard` now preserves AE9 across a rebuild (banner no longer reds out, and a rebuild isn't mistaken for a new day). Deployed to all 9 targets + PWA pushed (commit `04b7098`). **One manual step outstanding:** Sebastian must run "Rebuild Home Dashboard" in each of the 8 live Sheets to install the open-trigger (one auth prompt each) — the PWA path is already live everywhere without it. After that, the most likely next code direction is the **ManageVendors edit-form UX redesign**.

### Next-session candidates (impact-ranked)

0. **Confirm the per-Sheet trigger installs landed** (HOUSEKEEPING). The sheet-open auto-reset only activates after "Rebuild Home Dashboard" is run per Sheet. Verify Sebastian completed all 8; flag any store still missing it.
...
2. **Parallelize `deploy.py`** (LOW). The audit's last item. `concurrent.futures.ThreadPoolExecutor` with per-target temp `.clasp.json` location. 30s+ → ~5s wall clock. Mechanical, bounded scope.
   - *Note:* before touching deploy.py, glance at `.claude/skills/mog-deploy-workflow/scripts/route.py` — it's the new deterministic deploy router and shouldn't need changes, but parallelization will alter deploy.py's internals it documents.
