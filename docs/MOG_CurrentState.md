...
1. **ManageVendors edit-form redesign** (MEDIUM-HIGH). Add Vendor uses the friendly delivery-day + lead-day picker; Edit Vendor still shows raw multipliers. Needs an architectural walkthrough first — the mults→delivery mapping isn't trivially reversible; may need a small schema migration (store `deliveryDays[]` + `leadDays` as columns, compute mults at save). Sebastian-flagged. **This is now the top remaining UX inconsistency.**
...
3. **Reconcile the Rhino-ES5 invariant** (HOUSEKEEPING). CLAUDE.md invariant #4 says modals are Rhino/ES5, but HtmlService modals render in the *browser* and ManageVendors.html + OrderHistory.html already use ES6 (`const`) in production. Update invariant #4 + the `rhino-safe-html` skill to match reality.
...
*(Done this session: StorageAreas draft mode + 4 standalone PWA/data improvements. The pre-2026-05-28 "StorageAreas draft-mode UX" candidate is now closed.)*
