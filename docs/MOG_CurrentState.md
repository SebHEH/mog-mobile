...
**Skills infrastructure matured (2026-05-26) — codebase work itself unchanged.** The MOG skill set grew from 4 to 7 (added `mog-rpc-consolidation`, `mog-apps-script-caching`, `mog-modal-ux-sweep` — patterns that were being re-derived each session) and then got a full audit: visibility frontmatter (`disable-model-invocation` on add-store; `user-invocable:false` on the 3 pattern skills), 3 deterministic helper scripts (`route.py` deploy router, `validate.py` add-store ID/URL checks, `audit_modals.py` modal drift detector), and de-duplication of the canary/redeploy logic that was restated across 5 skills into a single `route.py` source of truth. No code reached the stores — this was all `.claude/` tooling. **On the actual codebase, the Apps Script modal perf audit remains effectively complete** (6 of 7; only item #6, parallelize `deploy.py`, is left). The most likely next code direction is still the **ManageVendors edit-form UX redesign** Sebastian flagged: the add-form uses a "pick delivery days + lead days" model but the edit-form shows raw multipliers — inconsistent and confusing.
