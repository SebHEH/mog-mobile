...
**ManageItems multi-vendor + declutter shipped (2026-05-27 later session) — live on all 9 stores.** Items can now be orderable from multiple vendors (eligible list in `MASTER_ITEMS` column **O** — a NEW column; SKU in D was left alone because every vendor tab `XLOOKUP`s D for its hidden SKU display column) with a one-tap "Active vendor" quick-switch on the View detail. `commitSwitchActiveVendor` moves the item to that vendor's tab; **par is shared per-item, the vendor's day-multiplier does the rest** — verified math-safe against the real RP_ROSSLYN_FOH sheet formulas. Declutter (all 6 asks): removed the Edit tab + vestigial selectors (and an RPC — `loadItemIntoEdit` now uses `getItemForEdit`), reworded the legend (🟡 Possible under-ordering / 🔴 Possible over-ordering / No storage area, dropped Inactive), no-scroll table, Inactive/Unassigned tab count chips, green Add panel, and removed the modal's self-added title+✕ that doubled Google's `showModalDialog` chrome. Bound-sidebar push (no `--redeploy`), canary rprfo first. **Per-store TODO:** run Ordering Guide → Mobile API → **Migrate Item Vendors** on each sheet when convenient (optional sheet hygiene — reads self-heal). **CAVEAT:** rpr (Rosslyn BOH) pars may not be true 1-day pars; Sebastian to recalc before using the switch there. **Next code direction: the modal-chrome de-dup sweep** across the other modal-dialog modals (Sebastian-approved for "afterwards").

### Next-session candidates (impact-ranked)

1. **Modal-chrome de-dup sweep** (Sebastian-approved next). The other `showModalDialog` modals (OrderHistory first; then check ManageVendors / StorageAreas / ReorderPickPath / AdminReset / HowToUse) likely add their own title + ✕ that doubles Google's chrome — same fix just applied to ManageItems (remove the self-added title bar text + ✕, keep Google's single frame). `mog-modal-ux-sweep` applies.
2. **Finish the modal consistency sweep** (MEDIUM). Close-affordance shipped earlier. Remaining: header/title markup (three different skeletons) and footer primary-action button naming/styling. Both touch every modal's markup and need a canonical-version decision first. `audit_modals.py` is green on its 3 axes; this is the drift it doesn't cover.
